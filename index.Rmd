...
Vamos aplicar médias móveis e Loesss em um mesmo conjunto de dados que fala sobre ICV, índice de custo de vida no município de São Paulo nas seguintes variáveis
...
Vamos aplicar o método Loesss com p = 0,08 no mesmo conjunto de dados para comparar os resultados
...
g1_lw = ggplot(Dados_ICV, aes(x = `Mes/ano`, y = ICV)) +
  ...
  theme_classic() +
  labs(title = "Dados ICV",
  subtitle = "Com Loesss considerando p = 0,08",
  x="Data",
  y="ICV",
  fill="")
...
g2_lw = ggplot(Dados_ICV, aes(x = `Mes/ano`, y = log_ICV)) +
  ...
  theme_classic() +
  labs(
  subtitle = "Em logaritmo com Loesss considerando p = 0,08",
  x="Data",
  y="ICV",
  fill="")
...
Tentaremos retirar a tendência a partir do ajuste do método Loesss considerando p = 0,08, exploremos os resultados

```{r, echo = FALSE, warning = FALSE, message=FALSE, out.width = "100%"}

g3_lw = ggplot(Dados_ICV, aes(x = `Mes/ano`, y = ICV_lw3)) + geom_path() + theme_classic() + 
  labs(title = "Dados ICV sem tendência",
  subtitle = "Método Loesss",
  x="Data",
  y="ICV",
  fill="")
...
g4_lw = ggplot(Dados_ICV, aes(x = `Mes/ano`, y = log_ICV_lw3)) + geom_path() + theme_classic() +
  labs(
  subtitle = "Com logaritmo por Loesss",
  x="Data",
  y="log(ICV)",
  fill="")
...
Vamos explorar esses gráficos que comparam as médias móveis de terceira ordem e o método Loesss com p = 0,08
...
Nesse gráfico abaixo conseguimos ver de forma bem mais clara como é a construção do método Loess utilizando o mesmo conjunto de dados PFI

```{r, echo = FALSE, warning = FALSE, message=FALSE, out.width = "100%"}

# Gráfico da série com loess
g1_lw0 = ggplot(Dados_PFI, aes(x = tempo, y = PFI)) +
  geom_path() +
  geom_smooth(method="loess", se=FALSE, span= .03, col="blue") + theme_classic() + 
  labs(title = "Dados PFI",
  subtitle = "Método Loess sendo p = 0,03",
  x="Meses",
  y="PFI",
  fill="")
...
g2_lw1 = ggplot(Dados_PFI, aes(x = tempo, y = PFI)) +
  ...
  labs(
  subtitle = "Método Loess sendo p = 0,1",
  x="Meses",
  y="PFI",
  fill="")
...
g3_lw2 = ggplot(Dados_PFI, aes(x = tempo, y = PFI)) +
  ...
  labs(
  subtitle = "Método Loess sendo p = 0,5",
  x="Meses",
  y="PFI",
  fill="")
